...
public class DynamicWeightingAStarSearch
{
	...
	public List<GraphNode> find_path(GraphNode start, GraphNode goal)
	{
		...
		while(fringe.child.Count > 0)
		{
			...
			Debug.Log("fringe child 0 position " + fringe.child[0].position.ToString("F4") +
						" neighbour num: " + fringe.child[0].neighbours.Count);
			

			if(current == goal)
			{
				Debug.Log("Parent was goal, yay!");
				GraphNode parent = fringe.parent[0];
				path.push(current, parent, current_g, weight);
			...
			*/
			...
			Debug.Log("Current neighbours size: " + current.neighbours.Count);
			for(int i = 0; i < current.neighbours.Count; i++)
			{
				Debug.Log("i i i i i i");
	...
	private List<GraphNode> reconstruct_path(GraphQueue path)
	{
		List<GraphNode> final_path = new List<GraphNode>();

		for(int i = 0; i < path.child.Count; i++)
		{
			final_path.Add(path.child[0]);
		}
